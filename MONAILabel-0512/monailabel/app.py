...
# Handle OHIF assets requested without /ohif/ prefix
# Some assets are requested as /assets/... instead of /ohif/assets/...
@app.get("/assets/{path:path}", include_in_schema=False)
async def get_ohif_assets(path: str):
    """Serve OHIF assets that are requested without the /ohif/ prefix."""
    ohif_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "endpoints", "static", "ohif")
    file_path = os.path.join(ohif_dir, "assets", path.replace("/", os.sep))
    
    if not os.path.exists(file_path):
        # Fallback to index.html for SPA routing
        file_path = os.path.join(ohif_dir, "index.html")
